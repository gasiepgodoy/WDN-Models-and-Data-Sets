...
folder='torch_data/'
Save_folder ='work_dir/'   #folder containing the torch files
...
parser.add_argument('--k', type=int, default=10, help='number of k-folds') 
...
folderPath1 = os.path.join(fileDirectory, 'work_dir2') # output folder to training/val statistics
...
def train_epoch(model, device, trainLoader, loss_fn, optimizer, batch_size, adj_matrices):    
    ...
    for epoch in range(num_epochs):
        ...
        ### uncomment below to show each prediction of the GNN
...
def CONTROL(T1,TEMPO_AQ, COUTER):  #ensures that each time class will have the same number of trainings
    c=0
...
RESULT = {'train':[], 'val':[], 'results':[]} # saves historical data
...
if __name__ == '__main__':
    ...
    for idx in range(len(D_NAME)):
        ...
        for folder in range(len(folders)):
            ...
            # create a binary pickle file to store results
            f = open("results/case1.pkl","wb")
