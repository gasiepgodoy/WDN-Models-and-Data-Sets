...
def Tensor_Dataset(x, y):
    ...
    A1 = torch.LongTensor(A)                  # Adj. Matrix Location
...
dataPath = os.path.join(fileDirectory, '.\Original_Data')   # folder containing the CSV files
...
folderPath = os.path.join(fileDirectory, './work_dir') # folder for the output tensor files  
...
for folders in range(len(namelist)):   
    ...
    Mcj = pd.read_csv(dataPath + '/Mat-adj-com-junc.csv', header=None) #adjacency matrix
    
 
    #columns of the CSV dataset containing:
      
    pa = 9  #start of pressure data
    pb = 17 #end of pressure data
    la = 49 #start of leakage nodes labels
    lb = 54 #end of leakage nodes labels
